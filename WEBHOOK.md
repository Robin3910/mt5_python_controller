...
无论哪个模版，信号本身只提供「品种 + 方向」，策略参数一律来自后台配置的规则快照——**不能通过 Webhook 传网格区间、风险金额之类的参数**。
...
上例中节点会：按风险金额与 `2397.0` 反推总手数 → 底仓市价成交（止损 `2397.0`、止盈按盈亏比挂出）→ 价格达到各批触发价时补齐剩余仓位（共用同一组止损止盈，总风险不变）→ 若开了保本触发，浮盈达标后把止损移到持仓加权均价。

> 模版2 的信号里 `volume`、`tp` 都会被忽略：手数由风险金额决定，止盈由盈亏比决定。

#### 模版3：网格交易

复刻币安现货手动创建网格。信号只负责「启动这组网格」，区间 / 格数 / 每格手数 / 止损止盈都取自策略规则，因此**不要求携带 `sl`**：

```json
{"model": "strategy", "action": "buy", "symbol": "XAUUSD"}
```

节点收到后会：按区间与格数切出网格线 → 若配了触发价则等到价、否则立即启动 → 可选按现价上方格位市价建底仓 → 之后价格下跌穿越网格线就买入一格、上涨穿越就卖出对应格。**空仓是正常运行态**（涨出区间顶部时会全部卖光，等回落再买），所以网格任务不会因持仓归零而收口，只由止损价 / 止盈价 / `CLOSE` 信号结束。
...
| `follow` 跟随信号 | `BUY` / `SELL` 均可，按信号定方向 |

开了**向上追踪**的网格，价格越过区间外沿时不会停机，而是把整个区间连同止损止盈平移一格继续跑（多头追涨、空头追跌），直到触发止损或达到配置的最大平移格数。

> 模版3 的信号里 `volume`、`sl`、`tp` 都会被忽略：手数按每格手数下单，止损止盈来自网格规则。
