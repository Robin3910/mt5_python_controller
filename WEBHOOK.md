...
可选的**分组趋势风控**（管理端「分组」页按组开启，默认关）：开启后，开仓信号在该节点抢到占位、建子任务前，会按「趋势面板」**全局参数**读取该节点终端行情并算趋势——`BUY` 仅多头放行、`SELL` 仅空头放行；中性、数据不足或行情读取失败一律拦截，子任务记为 `skipped` 并写中文 `skip_reason`；组内无人下发时主任务「未下发原因」与 `tasks[].reason` 同步为趋势文案（不再误写成「节点均有进行中的任务」）。`CLOSE` / 停策略**绕过**该门禁。组内 `poll` 模式下被拦节点会顺延下一节点。Webhook 本身无额外字段；是否拦截取决于分组配置，不写进信号体。

策略实例基于**策略模版**创建，不同模版对信号的要求不同：

| 模版 | 首单手数来源 | 对信号的额外要求 |
| --- | --- | --- |
| 策略模版1「AI智能加仓策略」（逆势 / 顺势加仓） | 信号 `volume` | 无 |
...
节点收到后不再按现价成交，而是一次挂出一整组限价单：底仓与分散仓**全部挂在 `limit_price`**（同一点位）。止损共用信号 `sl`，分散仓仍按该入场价铺阶梯止盈（底仓不挂止盈）。挂单一直等到成交（GTC），期间该节点该品种不接新信号。

手数口径与市价相同：按入场价到止损价的距离反推，不再因「越靠近止损距离越短」而放大仓位。
...
- `tasks[].status` 常见值：`dispatching`（已下发）/ `skipped`（组内无有效节点、节点均忙、趋势风控全拦等）/ `failed`（主任务创建失败或连接不可用等）。
- 开启了分组趋势风控时，被拦节点记为子任务 `skipped`（中文 `skip_reason`）；若该组一个节点都没发出去，主任务 `skip_reason` / `tasks[].reason` 与子任务原因一致（趋势拦截不会再显示成「均有进行中的任务」）。Webhook 顶层仍可能是 `accepted`（其它节点或其它分组已发出），需在分组信号页核对。
