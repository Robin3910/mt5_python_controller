...
> 注：这里的 `volume` 仅是「信号携带手数」。最终每个节点实际下单手数还取决于该节点的手数策略（跟随全局 / 固定 / 跟随信号），见节点配置。**策略模版2 是例外**：它按风险金额反推手数，完全不使用信号 `volume`。
...
模版2 的手数由「风险金额 ÷ 止损距离」算出，没有止损价就算不出手数，因此缺 `sl` 的信号会在分发前被挡下，落选原因写进信号记录（响应 `mode=rejected`）。同品种下若还有模版1 的分组，它不受影响、照常入选。
