...
def test_create_addon_keeps_manual_scatter(client):
    ...
    r = client.post(
        ...
        json={
            ...
            "rules": [
                ...
                {"type": 2, "status": 1, "action": "all",
                 "manual_scatter": _manual_scatter(volume=0, volume_locked=False, close_all_on_tp=False)},
            ],
        },
        headers=h,
    )
    ...
    # 止盈联动清仓：不传默认开启，显式传 False 原样落库
    assert by_type[1]["manual_scatter"]["close_all_on_tp"] is True
    assert by_type[2]["manual_scatter"]["volume_locked"] is False
    assert by_type[2]["manual_scatter"]["close_all_on_tp"] is False
...
def test_normalize_manual_scatter_defaults_disabled():
    ...
    assert out["manual_scatter"]["close_all_on_tp"] is True
    # 旧配置缺该字段：规范化后按默认开启；显式关闭保留
    legacy = tpl.normalize_rule({
        "type": 2, "status": 1, "action": "all",
        "manual_scatter": {"enabled": True, "entry_price": 4400, "take_profit": 4410},
    })
    assert legacy["manual_scatter"]["close_all_on_tp"] is True
    off = tpl.normalize_rule({
        "type": 2, "status": 1, "action": "all",
        "manual_scatter": {"enabled": True, "entry_price": 4400, "take_profit": 4410, "close_all_on_tp": 0},
    })
    assert off["manual_scatter"]["close_all_on_tp"] is False
